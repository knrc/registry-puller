...
func (wh *Webhook) createOrUpdateWebhookConfig() {
	if wh.webhookConfiguration == nil {
		logError.Print("mutatingwebhookconfiguration update failed: no configuration loaded")
		return
	}
...
func (wh *Webhook) rebuildWebhookConfig() error {
	...
	if wh.webhookConfiguration == nil || !reflect.DeepEqual(webhookConfig, wh.webhookConfiguration) {
		if err != nil {
			logError.Printf("mutatingwebhookconfiguration (re)load failed: %v", err)
			return err
		}
		...
		logInfo.Printf("%v mutatingwebhookconfiguration (re)loaded: \n%v",
...
// cleaned up when the deployment is deleted.
func rebuildWebhookConfigHelper(
	...
	// update ownerRefs so configuration is cleaned up when the deployment is deleted.
...
func (wh *Webhook) reloadKeyCert() {
	...
	if err != nil {
		logError.Printf("Cert/Key reload error: %v", err)
		return
	}
	wh.mu.Lock()
	wh.cert = &pair
	...
	logInfo.Printf("Cert and Key reloaded")
}
